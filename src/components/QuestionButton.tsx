...
const QuestionButton = ({ isAnswered, setIsCorrect, setIsAnswered }: Props) => {
  return (
    <button
      ...
      className={`bg-primary relative isolate flex min-w-[20.4375rem] items-center justify-center overflow-hidden rounded-[12px] p-4 font-medium transition-colors duration-300 before:absolute before:inset-0 before:bg-white/50 before:opacity-0 before:transition-opacity before:duration-300 hover:cursor-pointer hover:before:opacity-100 md:min-w-[40rem] md:rounded-[24px] md:p-8 xl:min-w-[35.25rem]`}
    >
