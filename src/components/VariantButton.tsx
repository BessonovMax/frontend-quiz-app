...
const VariantButton = ({ isAnswered, isCorrect, option, letter }: Props) => {
  return (
    ...
    >
      <div
        className={`bg-variant-foreground text-text-letter flex size-[2.5rem] items-center justify-center rounded-[6px] text-[1.125rem] uppercase md:size-[3.5rem] md:rounded-[12px] md:text-[1.75rem] xl:rounded-[8px] ${isCorrect === false ? "!bg-incorrect text-white" : ""}`}
      >
        {letter}
      </div>
